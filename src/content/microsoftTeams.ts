...
export function isTeamsParticipantElement(el: HTMLElement): boolean {
  if (
    el.closest(
      '[data-tid*="participant" i], [data-tid*="roster" i], [data-tid*="stage" i], [data-tid*="video-tile" i], [data-tid*="grid-tile" i]'
    )
  ) {
    return true;
  }
  ...
  const combined = `${aria} ${title} ${tooltip}`;

  if (
    combined.includes("'s microphone") ||
    combined.includes("'s mic") ||
    combined.includes("'s camera") ||
    combined.includes("'s video")
  ) {
    return true;
  }
...
export function isIgnoredTeamsChatOrHubElement(el: HTMLElement): boolean {
  if (
    el.closest(
      '[data-tid*="chat-header" i], ' +
        '[data-tid*="chat-input" i], ' +
        '[data-tid*="compose" i], ' +
        '[data-tid*="message-pane" i], ' +
        '[data-tid*="channel-header" i], ' +
        '[data-tid*="meet-landing" i], ' +
        '[data-tid*="meet-hub" i]'
    )
  ) {
    return true;
  ...
  if (
    combined.includes("start video call") ||
    combined.includes("start audio call") ||
    combined.includes("start a call") ||
    combined.includes("record a video") ||
    combined.includes("record an audio") ||
    combined.includes("record video") ||
    combined.includes("record audio") ||
    combined.includes("open video filter") ||
    combined.includes("attach video") ||
    combined.includes("create a meeting link") ||
    combined.includes("schedule a meeting") ||
    combined.includes("join with a meeting id") ||
    combined.includes("share link")
  ) {
    return true;
  }
